    ...
    export default {
        data() {
            return {
                ...
                objectLoader: null,// 外部模型加载器 .GLTF，.GLB
                JSONLoader: null,// 外部模型加载器 .GLTF，.GLB
                cssRender: null,
                videoComponent: null,
        ...
        methods: {
            ...
            initLoader() {
                ...
                this.GLTFLoader.load('static/three/model/scene.gltf', function (obj) {
                        cameraObj = obj.scene;
                        vm.GLTFLoader.load('static/three/model/温湿度/scene.gltf', function (obj) {
                                sensorObj = obj.scene;
                                // 本地数据
                                vm.initData(room3D);
                        }, undefined,
            ...
            createSense() {
                ...
                this.senseList.forEach(item => {
                    if (item.type === 'camera') {
                        this.cameraNum++;
                        this.addCamera(item.data)
                    } else {
                        this.sensorNum++;
                        this.addScene(item.data)
                    }
                })
                // group.add(geometry)
            },
            ...
            getMaterial(item) {
                ...
                if (item.type === 'cabinet') {
                    ...
                    bgcolor = '#ddd';
                    textcolor = '#000';
                    texture = new THREE.TextureLoader().load('static/images/ODF.png');
                }
                //导入材质
